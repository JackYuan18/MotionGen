...
from utils import get_next_downsampled_state
...
def downsample_states(states: torch.Tensor, ratio: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if len(states.shape) == 2:
        ...
        indices = torch.arange(0, seq_len, ratio, device=states.device, dtype=torch.long)
        return states[::ratio], indices
    else:
...
def train_epoch_ppo(
    ...
    with torch.no_grad():
        ...
        # Compute average step reward for monitoring
        avg_step_reward = step_rewards.mean().item()
        min_step_reward = step_rewards.min().item()
        max_step_reward = step_rewards.max().item()
        ...
        # Expand downsampled_indices for each rollout
        downsampled_indices_expanded = downsampled_indices.expand(num_rollouts, -1) if downsampled_indices.shape[0] == 1 else downsampled_indices
        downsampled_indices_flat = downsampled_indices_expanded.repeat_interleave(predicted_seq_len, dim=0)  # [num_rollouts * predicted_seq_len, seq_len_downsampled]
        ...
        for batch_start in range(0, total_states, value_batch_size):
            ...
            # Find next downsampled states for this batch
            
            batch_next_downsampled_states, batch_next_time_indices = get_next_downsampled_state(
                time_indices_flat[batch_slice],
                actual_traj_flat[batch_slice],
                downsampled_indices_flat[batch_slice]
            )
            
            batch_values = value_net(
                batch_next_downsampled_states,
                batch_next_time_indices,
    ...
    for ppo_epoch in range(ppo_epochs):
        ...
        for batch_start in range(0, total_samples, batch_size):
            ...
            rollout_indices = batch_indices // predicted_seq_len  # Which rollout
            ...
            # Expand downsampled_indices to match batch size if needed
            batch_downsampled_indices = downsampled_indices
            if batch_downsampled_indices.shape[0] == 1:
                batch_downsampled_indices = batch_downsampled_indices.expand(num_rollouts, -1)
            # Select the correct downsampled_indices for each rollout in the batch
            batch_downsampled_indices_selected = batch_downsampled_indices[rollout_indices]  # [batch_size, seq_len_downsampled]
            
            # Find next downsampled states for each batch element
            
            batch_next_downsampled_states, batch_next_time_indices = get_next_downsampled_state(
                batch_time_indices, batch_actual_traj, batch_downsampled_indices_selected
            )
            
            mean, log_std = actor.forward(
                batch_next_downsampled_states,
                batch_next_time_indices,
                batch_states, 
                current_time_index=batch_time_indices
            )
            ...
            # Compute value estimates (reuse next downsampled states computed above)
            new_values = value_net(
                batch_next_downsampled_states,
                batch_next_time_indices,
                batch_states,
                current_time_index=batch_time_indices
    ...
    # Average std from actor policy
    avg_std = total_std_sum / total_std_count if total_std_count > 0 else 0.0
    ...
    return avg_loss, normalized_actor_loss, normalized_value_loss, rollout_trajectories, avg_step_reward, min_step_reward, max_step_reward, avg_std
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        # Train with PPO
        train_loss, actor_loss, value_loss, rollout_trajectory, avg_step_reward, min_step_reward, max_step_reward, avg_std = train_epoch_ppo(
            actor=actor,
            value_net=value_net,
        ...
        print(f"Train Loss: {train_loss:.6f}, Actor Loss: {actor_loss:.6f}, Value Loss: {value_loss:.6f}, "
              f"Avg Step Reward: {avg_step_reward:.6f} (min: {min_step_reward:.6f}, max: {max_step_reward:.6f}), "
              f"Avg Std: {avg_std:.6f}")
