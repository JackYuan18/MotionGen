...
def rollout_trajectory_deterministic_mean(
    ...
    # Create downsampled_indices from actual_trajectory (sequential indices)
    actual_seq_len = actual_trajectory.shape[1]
    downsampled_indices = torch.arange(actual_seq_len, device=device, dtype=torch.float32).unsqueeze(0).expand(batch_size, -1)
    
    # Import get_next_downsampled_state
    ...
    for t in range(seq_len):
        ...
        # New signature: forward(next_downsampled_state, next_time_index, current_state, current_time_index)
        mean, _ = actor.forward(
