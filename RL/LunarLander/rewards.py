...
def compute_reward_downsampled_next(
    ...
    # Safety check: ensure seq_len_downsampled is valid
    if seq_len_downsampled == 0:
        # Return zero rewards if no downsampled states
        return torch.zeros(batch_size, seq_len_pred, device=predicted_states.device)
    ...
    target_indices = torch.clamp(target_indices, 0, seq_len_downsampled - 1)
    ...
    heading_diff = state_diff[:, :, 4]
    state_diff[:, :, 4] = torch.atan2(torch.sin(heading_diff), torch.cos(heading_diff))*2
    ...
    if actions is not None:
        seq_len_actions = actions.shape[1]
        
        # Handle sequence length mismatch: take min of predicted and action lengths
        min_len = min(seq_len_pred, seq_len_actions)
        
        # Action smoothness: difference between consecutive actions (only for valid length)
        if min_len > 1:
            prev_actions = torch.zeros_like(actions[:, :min_len, :])
            prev_actions[:, 1:, :] = actions[:, :min_len-1, :]
        
        # Action norm: magnitude of actions (reduced penalty for first action)
        # Take only the relevant portion of actions to match predicted_states length
        actions_truncated = actions[:, :min_len, :]  # [batch_size, min_len, action_dim]
        action_norm_partial = (actions_truncated.abs() > 0).float().sum(dim=2)  # [batch_size, min_len]
        action_norm[:, :min_len] = action_norm_partial
        action_norm[:, 0] = action_norm[:, 0] * 0.1
