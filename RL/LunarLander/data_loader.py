...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            dataset_type: Type of dataset (vehicle or lunar)
        """
        self.states = states
        ...
        if normalize:
            if self.dataset_type == 'vehicle':
                self.normalize_vehicle_states(states)
            elif self.dataset_type == 'lunar':
                self.normalize_lunar_states(states)
            else:
    ...
    def normalize_lunar_states(self, states: np.ndarray) -> np.ndarray:
        ...
        if state_flat.shape[1] > 2:  # Ensure heading dimension exists
            ...
            self.state_std[4] = 1.0  # Heading std = 1 (no scaling)
            
            # Heading mean = 0 (no shift)
            self.state_mean[4] = 0.0
        else:
            # If no heading dimension, use max std as global scale
            global_scale = np.max(per_dim_std)
            self.state_std = np.full_like(per_dim_std, global_scale)

    def normalize_vehicle_states(self, states: np.ndarray) -> np.ndarray:
        state_flat = states.reshape(-1, states.shape[-1])
        self.state_mean = np.mean(state_flat, axis=0)
        per_dim_std = np.std(state_flat, axis=0) + 1e-8  # Add small epsilon
